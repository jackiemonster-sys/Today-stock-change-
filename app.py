...
# 修正：單支股票資料下載（解決抓不到資料與 MultiIndex 問題）
@st.cache_data(ttl=600)
def fetch_single_stock(symbol):
    try:
        # 改用 Ticker().history 減少線上平台封鎖機率
        t = yf.Ticker(symbol)
        ...
        # 統一首字母大寫以確保後續計算不報錯 (Open, High, Low, Close, Volume)
...
# ================= 模式二：個股轉折 K 線圖查詢 =================
    ...
    if stock_id:
        # 先嘗試上市 (.TW)，再嘗試上櫃 (.TWO)
        df = fetch_single_stock(f"{stock_id}.TW")
        if df.empty:
            df = fetch_single_stock(f"{stock_id}.TWO")
