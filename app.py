...
def analyze_turnaround(df, lookback_days=10):
    ...
    df["Vol_Ratio"] = (df["Volume"] / df["MA5_Vol"]).round(2)  # 成交量相較 5 日均量倍數
    df["N_Vol_Max"] = df["Volume"].rolling(20).max().shift(1)  # 近 20 日最大量

    # 爆大量定義：成交量大於 5 日均量 2 倍，且創近 20 日新高量
    df["Is_Volume_Spurt"] = (df["Volume"] >= df["MA5_Vol"] * 2.0) & (
        df["Volume"] > df["N_Vol_Max"]
    )
...
# ================= 模式二：個股轉折 K 線與成交量圖 (含爆量提醒) =================
else:
    st.title("📊 個股 A/V 轉折與成交量監控")
    ...
    if stock_id:
        ...
        if df.empty or len(df) < lookback + 1:
            ...
            if pd.notna(latest["MA5"]):
                ma_status.append(
                    f"5MA ({latest['MA5']:.2f}): "
                    + ("🟢 站上" if close_price >= latest["MA5"] else "🔴 跌破")
                )
            if pd.notna(latest["MA20"]):
                ma_status.append(
                    f"20MA月線 ({latest['MA20']:.2f}): "
                    + ("🟢 站上" if close_price >= latest["MA20"] else "🔴 跌破")
                )
            if pd.notna(latest["MA60"]):
                ma_status.append(
                    f"60MA季線 ({latest['MA60']:.2f}): "
                    + ("🟢 站上" if close_price >= latest["MA60"] else "🔴 跌破")
                )
            ...
            with col1:
                ...
                st.metric(
                    "成交量 (張)",
                    f"{vol_sheets:,}",
                    f"為 5MA 均量之 {latest['Vol_Ratio']} 倍",
                )
            ...
            with col2:
                ...
                # 爆量提醒
                if latest["Is_Volume_Spurt"]:
                    st.warning("⚠️ 警告：今日成交量觸發【爆大量】！")

            with col3:
                st.subheader("🛡️ 均線防守狀況")
                for status in ma_status:
                    st.caption(status)

            # 3. 繪製圖表：使用 Subplots 製作 主圖(K線) + 副圖(成交量)
            fig = make_subplots(
                rows=2,
                cols=1,
                shared_xaxes=True,
                vertical_spacing=0.03,
                subplot_titles=(f"{stock_id} K線與均線", "成交量 (張)與爆量訊號"),
                row_heights=[0.7, 0.3],
            )
            ...
            # --- 副圖：成交量柱狀圖 (爆大量顯眼化) ---
            # 依條件給予柱狀圖顏色：爆量用黃色/金黃，漲用紅，跌用綠
            vol_colors = []
            for _, row in df.iterrows():
                if row["Is_Volume_Spurt"]:
                    vol_colors.append("#FFD700")  # 爆大量：金黃色
                elif row["Close"] >= row["Open"]:
                    vol_colors.append("rgba(255, 0, 0, 0.5)")  # 上漲：淡紅
                else:
                    vol_colors.append("rgba(0, 128, 0, 0.5)")  # 下跌：淡綠
