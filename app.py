...
def analyze_turnaround(df, lookback_days=10):
    ...
    # 倍數計算
    df["Vol_Ratio_MA5"] = (df["Volume"] / df["MA5_Vol"]).round(
        2
    )  # 相較 5 日均量倍數
    df["Vol_Ratio_Prev"] = (df["Volume"] / df["Volume"].shift(1)).round(
        2
    )  # 相較昨日成交量倍數

    # 【爆大量判定】：1. 比昨日大 2 倍以上  OR  2. 比 5 日均量大 2 倍以上
    ...
    try:
        ...
        if latest["V_Turn"]:
            return {
                ...
                "較昨日": f"{latest['Vol_Ratio_Prev']} 倍",
                "較5日均量": f"{latest['Vol_Ratio_MA5']} 倍",
            }
        elif latest["A_Turn"]:
            return {
                "代號": ticker,
    ...
    if stock_id:
        ...
        if df.empty or len(df) < lookback + 1:
            ...
            with col2:
                ...
                # 爆量提醒 (大於昨日2倍 或 大於均量2倍)
                if latest["Is_Volume_Spurt"]:
                    st.warning("⚠️ 警告：今日成交量觸發【爆大量 2 倍以上】！")
            ...
            for _, row in df.iterrows():
                if row["Is_Volume_Spurt"]:
                    vol_colors.append("#FFD700")  # 爆大量：亮金黃色
