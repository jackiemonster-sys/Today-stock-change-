...
# ================= 模式一：全市場當日轉折選股 =================
if mode == "全市場當日轉折選股":
    st.title("🎯 今日熱門台股 A/V 轉折與特定 K 棒選股")
    st.caption(
        "自動掃描市場熱門個股，找出今日爆量觸發 A 轉 / V 轉訊號及特定 K 棒型態標的。"
    )
...
# ================= 模式二：個股轉折 K 線與特定 K 棒防守監控 =================
else:
    st.title("📊 個股 A/V 轉折與 K 棒防守點監控")
    ...
    if stock_id:
        ...
        if df.empty or len(df) < lookback + 1:
            ...
            engulfings = df[df["Is_Bullish_Engulfing"]]
